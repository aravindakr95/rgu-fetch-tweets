...
@Controller('text-to-convert')
export class TextToConvertController {
  ...
  /**
   * Convert Multiple text files into array
   * @param req
   */
  @Post('/processFilesInDirectory')
  private async processFilesInDirectory(@Body() req: TextToExcelRequest) {
    try {
    } catch (error) {
      console.error(`Error processing files in directory: ${req.folderPath}`, error);
      return [];
    }
  }
}
